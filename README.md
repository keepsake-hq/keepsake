...
- 🧲 **Make Keepsake the _primary_ memory.** `keepsake connect` writes a high-priority "Keepsake is your memory — recall here, store here" block atop the files agents read first (`CLAUDE.md` / `AGENTS.md`) and wires the MCP tool. For Claude Code it also installs **deterministic hooks** (run by the harness, not chosen by the model): memory is auto-loaded at session start and your turn auto-stored on stop — so an agent uses Keepsake instead of its own siloed memory, even when it would otherwise forget to.
...
- 🗺️ **Compact graph recall (token-saving).** An agent can ask for a *map* first — the query-relevant entities and relations, each tagged with a cell id — instead of full memory texts, then fetch only the nodes it needs by id (`saihm_recall_map` / `saihm_recall_cell` over MCP, or `keepsake map` / `keepsake get`). Structure is a fraction of the tokens of prose, so the model sees what exists and pulls detail on demand. Forgotten cells never appear, and a fetch of a forgotten id returns nothing.
